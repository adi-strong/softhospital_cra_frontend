...
import {useAddNewBedroomCategoryMutation} from "./bedroomCategoryApiSlice";
import toast from "react-hot-toast";

export const AddBedroomCategories = ({onHide, show = false}) => {
  const [categories, setCategories] = useState([{name: ''}])
  const [addNewBedroomCategory, {isLoading}] = useAddNewBedroomCategoryMutation()
  ...
  async function onSubmit(e) {
    if (categories.length > 0) {
      ...
      for (const key in categories) {
        const formData = await addNewBedroomCategory(categories[key])
        if (!formData.error) {
