...
const PersonalImageItem = ({id, selectedTarget, onSelected, setState}) => {
  ...
  useEffect(() => {
    if (check && image) setState({id: `/api/personal_image_objects/${image.id}`, contentUrl: image.contentUrl})
    else setState(null)
...
export const AddPersonalImageModal = ({show, onHide, item, itemState, setItemState}) => {
  const {data: images = [], isLoading, isSuccess, isError, isFetching, refetch} = useGetPersonalImagesQuery('PersonalImages')
...
AddPersonalImageModal.propTypes = {
